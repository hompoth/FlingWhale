...
public class ThirdPersonController : MonoBehaviour
{
    ...
    void UpdateSmoothedMovementDirection()
    {
		...
		if(h>0) _characterState = CharacterState.Right;
		else if(h<0) _characterState = CharacterState.Left;
		if(v>0) _characterState = CharacterState.Up;
		else if(v<0) _characterState = CharacterState.Down;
		if(v==0 && h==0) _characterState = CharacterState.Idle; 
        ...
        if (grounded)
        {
            ...
            if (targetDirection != Vector3.zero)
            {
                ...
                {
					moveDirection = targetDirection;//Vector3.RotateTowards(moveDirection, targetDirection, rotateSpeed * Mathf.Deg2Rad * Time.deltaTime, 0);
